...
iloc_map = function(iloc,
  ...
  if(postcodes == T){ #if we are searching for postcodes, set up the shapefile
    poa = raster::intersect(shapes, iloc::shape_poa)
  }

sscnames = tibble::tibble(id = shapes@data$SSC_CODE16, name = shapes@data$SSC_NAME16) %>%
...
ssc_data_f = iloc::data_ssc %>%
  dplyr::select(id = SSC_CODE_2016, males = Indigenous_P_Tot_M, females = Indigenous_P_Tot_F, total = Indigenous_P_Tot_P) %>%
...
suburb_data = shp_tb %>%
  dplyr::select(suburb = name, women = females, men = males, persons = total) %>%
  unique %>%
...
if(postcodes == T){
  ...
  pb = txtProgressBar(min =0, max = length(suburb_data$suburb), style = 3)
  suburb_data$postcodes = lapply(seq_along(suburb_data$suburb), function(x){
    pc = find_postcodes(suburb_data$suburb[x],simple_shapes = shapes,simple_poa = poa)
...
demographics = tibble(IL_code = iloc_f@data$ILOC_CODE, IL_name = iloc_f@data$ILOC_NAME) %>%
  unique

data_iloc = iloc::data_iloc %>%
  dplyr::select(IL_code = ILOC_CODE_2016,
                f_0_4 = A_0_4_Indig_F,
                m_0_4 = A_0_4_Indig_M,
...
map = get_stamenmap(bbox = c(left = min(shp_tb$long, na.rm=T) - gap,
...
final_map = ggmap(map) +
  geom_polygon(data = shp_tb,aes(x = long , y = lat,group = group, fill = total), alpha = .8) +
  geom_path(data = shp_tb,aes(x = long , y = lat,group = group, color = female_p), size = .3) +
  geom_path(data = iloc_outline,aes(x = long, y = lat, group = group), color = "black",size = .7)+
  geom_label_repel(
    data = suburb_data %>%
      data.frame() %>%
      mutate(limit = as.numeric(as.character(scale(persons)))) %>%
  ...
  labs(fill = "population\nsize 15+\n",
       caption = paste0("Note: total population = ", sum(suburb_data$persons, na.rm=T)),
       color = "percent\nfemale",
       x = " ",
       y = " ") +
  theme(axis.line = element_line(color = "black"),plot.caption = element_text(hjust=0))+
  ggtitle(" ")
